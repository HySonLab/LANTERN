...
from data_loader import DataProcessor, DDI_DataProcessor, DrugProteinDataSet
...
def main() :
    ...
    """
    train_data = DDI_DataProcessor(args.train_path, hop, mode="train")
    train_entity2index = train_data.entities2id 
    valid_data = DDI_DataProcessor(args.valid_path, hop, mode="val")
    valid_entity2index = valid_data.entities2id
    test_data = DDI_DataProcessor(args.test_path, hop, mode="test")
    ...
    train_data_loader = DataLoader(dataset=train_set, batch_size=512, shuffle=True, collate_fn=custom_collate)

    valid_set_len = len(valid_triples_data)
    valid_set = DrugProteinDataSet(valid_triples_data, args.neg_ratio)
    valid_data_loader = DataLoader(dataset=valid_set, batch_size=512, shuffle=True, collate_fn=custom_collate)

    test_set_len = len(test_triples_data)
    test_set = DrugProteinDataSet(test_triples_data, args.neg_ratio)
    test_data_loader = DataLoader(dataset=test_set, batch_size=512, shuffle=True, collate_fn=custom_collate)
    ...
    model = DDI_Model(entity_num, drug_num, protein_num, relation_num, args.embed_dim,
